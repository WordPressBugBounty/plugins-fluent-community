...
class DummyCommands
{
    ...
    public function create_posts($args, $assoc_args)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $randomDate = gmdate('Y-m-d H:i:s', mt_rand(strtotime('-1 years'), current_time('timestamp')));
    ...
    public function create_comments($args, $assoc_args)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $commentData = [
                ...
                'content_type'     => 'text',
                'status'           => 'published',
                'created_at'       => gmdate('Y-m-d H:i:s', mt_rand(strtotime('-1 years'), current_time('timestamp'))),
                'updated_at'       => gmdate('Y-m-d H:i:s', mt_rand(strtotime('-1 years'), current_time('timestamp'))),
            ];
    ...
    protected function getRandomStatus($minWords = 80, $maxWords = 300)
    {
        ...
        $sentences = [
            ...
            "Space-driven initiatives can lead to substantial local changes and **empowerment**.",
